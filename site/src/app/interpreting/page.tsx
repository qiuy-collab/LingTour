...
/**
 * Image pools sourced from the backend image library. Slot 0/1/2 cover the
 * three service-mode cards and three profile cards respectively; if the API
 * returns more entries than the pool has, indexes cycle.
 */
...
/**
 * Pricing matrix is now driven by i18n keys instead of hardcoded
 * locale-conditional strings, so adding a third language is a translations-only
 * change.
 */
...
export default function InterpretingPage() {
  ...
  // Tiered profile labels come from i18n now — no inline {en, zh} literals.
  ...
  }, [interpretingData?.serviceModes]);

  const showcaseProfiles = useMemo<InterpreterProfile[]>(() => {
    const profiles = interpretingData?.profiles ?? [];
    return profiles.map((profile, index) => {
      ...
      return {
        ...
        rateLabel: interpretingData?.serviceModes?.[index]?.price,
        bestFor: profile.helps?.join(" / "),
      };
    });
  }, [interpretingData?.profiles, interpretingData?.serviceModes, profileLevels]);

  if (loading) return <LoadingSpinner text="Setting up the interpreting desk…" />;
  if (error)
    return (
      <ErrorState
  ...
  return (
    <div className="bg-[var(--paper-deep)] bg-grain min-h-screen text-[var(--river-deep)]">
      <section className="relative min-h-[70vh] flex items-center overflow-hidden pt-32 pb-20">
        ...
        {/* Cinematic Narrow Strip — sourced from backend image library */}
