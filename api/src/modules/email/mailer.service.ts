...
export class MailerService {
  ...
  /**
   * Renders the event template (database override first, built-in default
   * otherwise) and sends it. Returns false on any failure so callers keep
   * their existing delivery-fallback semantics.
   */
  async sendTemplated(
    eventKey: string,
    to: string,
    ...
    // Only the caller-provided variables are rendered; anything missing
    // becomes an empty placeholder, never the preview example values.
    ...
    try {
      ...
      await transporter.sendMail({
        ...
        text: renderEmailTemplate(bodyHtml, mergedVars).replace(
          /<[^>]+>/g,
          ' ',
        ),
        html: renderEmailTemplate(bodyHtml, mergedVars),
      });
      return true;
    } catch (error) {
      this.logger.error(
